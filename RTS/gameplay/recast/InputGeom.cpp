...
bool InputGeom::loadMesh(rcContext* ctx, const std::string& filepath)
{
	...
	if (!m_mesh && ctx != nullptr)
	{
		ctx->log(RC_LOG_ERROR, "loadMesh: Out of memory 'm_mesh'.");
		return false;
	}
	if (!m_mesh->load(filepath) && ctx != nullptr)
	{
	...
	if (!m_chunkyMesh  && ctx != nullptr)
	{
		ctx->log(RC_LOG_ERROR, "buildTiledNavigation: Out of memory 'm_chunkyMesh'.");
		return false;
	}
	if (!rcCreateChunkyTriMesh(m_mesh->getVerts(), m_mesh->getTris(), m_mesh->getTriCount(), 256, m_chunkyMesh) && ctx != nullptr)
	{
