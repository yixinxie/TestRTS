...
void IntIntKVP::clear(){
	ZeroMemory(keyValuePairs, sizeof(OriGUID_kvp) * currentSize);
	occupied = 0;

}
void IntIntKVP::add(const OriGUID _key, int _value){
...
bool IntIntKVP::tryget(const OriGUID _key, int* outVal){
	int idx = hash_func(&_key);
	int originalIdx = idx;
	*outVal = 0;
	while (true){
...
void IntIntKVP::genGUID(OriGUID* guid){
	GUID result;
	UuidCreate(&result);
