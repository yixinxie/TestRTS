...
bool GLManager::init(HWND hWnd, int _width, int _height)
{
	...
	GLuint shaderHnd = loadShaders("shaders/sprite_vs.glsl", "shaders/sprite_fs.glsl");
	...
	IntVector2 textureDim = IntVector2(512, 512);
	...
	//CharHelper* pngRaw = CharHelper::loadBinaryFile("assets/arrows.png");
	assert(error == 0);
